...
class PPO:
    ...
    def load(self, loaded_dict: dict, load_cfg: dict | None, strict: bool) -> bool:
        ...
        if "model_state_dict" in loaded_dict and "actor_state_dict" not in loaded_dict:
            actor_state_dict, critic_state_dict = self._split_legacy_actor_critic_state_dict(
            ...
            # Legacy actor-critic checkpoints used a different optimizer parameter
            # layout. The policy weights are enough for play/evaluation.
            if load_cfg is not None:
                load_cfg = {**load_cfg, "optimizer": False}
    ...
    def _split_legacy_actor_critic_state_dict(
        self, model_state_dict: dict[str, torch.Tensor]
    ) -> tuple[dict[str, torch.Tensor], dict[str, torch.Tensor]]:
        """Convert pre-split actor-critic checkpoints to the current state dict layout."""
        actor_state_dict = {}
        critic_state_dict = {}
        actor_keys = self._raw_actor.state_dict().keys()

        for key, value in model_state_dict.items():
            if key == "std":
                if "distribution.std_param" in actor_keys:
                    actor_state_dict["distribution.std_param"] = value
                elif "distribution.log_std_param" in actor_keys:
                    actor_state_dict["distribution.log_std_param"] = torch.log(value)
