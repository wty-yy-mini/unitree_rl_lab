...
class Logger:
    ...
    @staticmethod
    def _collect_head_tags(repo: git.Repo) -> str:
        """Return tags that point at ``HEAD`` as a comma-separated string."""
        tags = [tag.strip() for tag in repo.git.tag("--points-at", "HEAD").splitlines() if tag.strip()]
            ...
            for repository_file_path in self.git_status_repos:
                ...
                with open(diff_file_name, "x", encoding="utf-8") as f:
                    content = (
                        ...
                        f"tags: {self._collect_head_tags(repo)}\n\n"
                        f"--- git status ---\n{repo.git.status()} \n\n\n"
                        f"--- git diff ---\n{repo.git.diff(t)}"
                    )
                    f.write(content)
